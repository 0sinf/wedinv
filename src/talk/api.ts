...
const SHEET_TALK_HEADER: (keyof SheetTalk)[] = ["id", "author", "party", "msg", "created", "password", "published"]
...
const serializeTalk = (talk: SheetTalk) => {
	return {
		...talk,
		created: new Date(talk.created).toISOString()
	}
}

const hashPasword = async (password: string) => ((await scrypt(password, "D7zboYc4Uc", 16)) as Buffer).toString("hex")
...
export const getTalkList = async (talkId: string) => {
	...
	allTalks.sort((a, b) => b.created.getTime() - a.created.getTime())
...
export const patchTalk = async (reqData: PatchTalkRequest) => {
	...
	const row = rows.find((r) => r.id === reqData.id && r.password === hashedPassword)
	if (!row) {
